...
static void drgn_program_update_platform(struct drgn_program *prog,
		       ...
		       const struct drgn_platform *platform)
{
	...
	prog->core_fd = -1;
	if (platform)
		drgn_program_update_platform(prog, platform);
}
...
drgn_program_set_pid(struct drgn_program *prog, pid_t pid)
{
	...
	prog->pid = pid;
	prog->flags |= DRGN_PROGRAM_IS_LIVE;
	drgn_program_update_platform(prog, &drgn_host_platform);
	return NULL;
...
static int drgn_update_platform_from_dwarf(Dwfl_Module *module,
					   void **userdatap, const char *name,
					   Dwarf_Addr base, Dwarf *dwarf,
					   Dwarf_Addr bias, void *arg)
{
	...
	if (!ehdr)
		return DWARF_CB_OK;
	drgn_platform_from_elf(ehdr, &platform);
	drgn_program_update_platform(arg, &platform);
...
struct drgn_error *drgn_program_update_dwarf_index(struct drgn_program *prog)
{
	...
	if (err)
		return err;
	if (!prog->has_platform) {
		dwfl_getdwarf(prog->_dwfl, drgn_update_platform_from_dwarf,
			      prog, 0);
	}
	return NULL;
}
